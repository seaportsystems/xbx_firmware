...
EZO_EC_I2CADDR_DEFAULT = const(0x64)
...
class EZO_EC:
    ...
    def probe_type(self):
        try:
            return self._parse_message(self._write_then_read('K,?', 600, 42))
        
        except:
            raise ConnectionError
        
    @probe_type.setter
    def probe_type(self, a):
        try:
            return self._write_then_read(f'K,{a}', 300, 42)
    ...
    def parameters(self):
        try:
            response = self._write_then_read("O,?", 300, 6)
    ...
    def parameters(self, config):
        if(len(config) != 4):
            raise ValueError("Parameter configuration must be a tuple of length 4")
        ...
        try:
            if(EC):
                self._write_then_read("0,EC,1", 300, 2)
            else:
                self._write_then_read("0,EC,0", 300, 2)
            
            if(TDS):
                self._write_then_read("0,TDS,1", 300, 2)
            else:
                self._write_then_read("0,TDS,0", 300, 2)
            
            if(S):
                self._write_then_read("0,S,1", 300, 2)
            else:
                self._write_then_read("0,S,0", 300, 2)
            
            if(SG):
                self._write_then_read("0,SG,1", 300, 2)
            else:
                self._write_then_read("0,SG,1", 300, 2)
        except:
            raise ConnectionError
    ...
    def EC(self):
        try:
            self.parameters = (True, False, False, False)
    ...
    def TDS(self):
        try:
            self.parameters = (False, True, False, False)
            return self.get_reading()[0]
    ...
    def S(self):
        try:
            self.parameters = (False, False, True, False)
    ...
    def SG(self):
        try:
            self.parameters = (False, False, False, True)
            return self.get_reading()[0]
    ...
    @property
    def tds_compensation(self):
        try:
            return self._parse_message(self._write_then_read('TDS,?', 300, 11))
        
        except:
            raise ConnectionError
    
    @tds_compensation.setter
    def tds_compensation(self, n):
        '''temperature for compensation in degrees celsius'''
        
        try:
            return self._write_then_read('TDS,{n}', 300, 2)
